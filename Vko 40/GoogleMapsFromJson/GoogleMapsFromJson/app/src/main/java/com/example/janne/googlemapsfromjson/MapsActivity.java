package com.example.janne.googlemapsfromjson;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void fetchData() {
        String url = "http://student.labranet.jamk.fi/~K1525/JSON/coords.json";
    ...
    class FetchDataTask extends AsyncTask<String, Void, JSONObject> {
        ...
        protected void onPostExecute(JSONObject json) {
            try {
                coords = json.getJSONArray("coords");
                for (int i=0;i < coords.length();i++) {
                    JSONObject naat = coords.getJSONObject(i);

                    lat = naat.getString("lat");
                    longi = naat.getString("long");
                    title = naat.getString("title");
